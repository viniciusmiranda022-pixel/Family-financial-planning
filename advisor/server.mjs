...
function interpretPrompt(payload) {
  return [
    ...
    "Quando a mensagem (ou o histórico, pela regra de continuação acima) indicar que uma despesa é parcelada ('parcelado', 'parcelada', 'em Nx', 'dividido em N vezes') mas sem dizer claramente qual é a intenção de registro do parcelamento, mesmo assim escolha create_expense e preencha installments_text com o texto exato dito sobre o parcelamento (ex.: 'parcelada', '10x', '10'); nunca deixe de extrair esse sinal só porque o número de parcelas ainda não apareceu -- o backend pergunta o número que faltar. Preencha monthly_interest_rate_text somente se a mensagem mencionar explicitamente juros/taxa do parcelamento; caso contrário deixe ausente (parcelamento sem juros).",
