...
function interpretPrompt(payload) {
  return [
    ...
    "Uma frase no passado relatando algo que já aconteceu ('gastei', 'paguei', 'recebi', 'transferi', 'coloquei mais X em', 'estornaram') É um pedido para registrar esse lançamento -- escolha a intenção de criação correspondente (create_expense/create_income/create_internal_transfer/pay_obligation/pay_card_invoice/register_refund/register_asset_contribution), exatamente como se a pessoa tivesse dito 'registre'/'lance'. Nunca trate esse relato como 'query' só porque não é um comando explícito no imperativo.",
    ...
    "Se o histórico mostrar que o turno anterior já era sobre a mesma intenção de registro (uma pergunta sua sobre valor/conta/categoria/parcelas/origem do recurso de uma saída/entrada/transferência/pagamento/estorno/aporte ainda não confirmado), trate a mensagem atual como continuação -- mantenha a mesma intenção e repita em extracted_fields cada campo já extraído nos turnos anteriores do histórico que a mensagem atual não contradiz, acrescentando apenas o que esta mensagem responde. Nunca reinicie do zero nem troque para 'query'/'unknown' só porque a mensagem atual sozinha, sem o histórico, pareceria incompleta ou ambígua.",
    ...
    "Quando a mensagem (ou o histórico, pela regra de continuação acima) indicar que uma despesa é parcelada ('parcelado', 'parcelada', 'em Nx', 'dividido em N vezes') mas sem dizer claramente qual é a intenção de registro do parcelamento, mesmo assim escolha create_expense e preencha installments_text com o texto exato dito sobre o parcelamento (ex.: 'parcelada', '10x', '10'); nunca deixe de extrair esse sinal só porque o número de parcelas ainda não apareceu -- o backend pergunta o número que faltar. Preencha monthly_interest_rate_text somente com o que a mensagem realmente disse sobre juros -- um valor/percentual explícito, ou uma negação explícita como 'sem juros'/'0%' (que também é um texto válido para esse campo: ausência de juros dita pelo usuário é um fato, não uma suposição sua). Se a mensagem não mencionar juros nem a ausência deles, deixe o campo ausente -- nunca escreva '0' ou 'sem juros' por conta própria só porque a mensagem não falou nisso; o backend trata a taxa como materialmente desconhecida e pergunta.",
...
function planPrompt(payload) {
  return [
    ...
    "Se a mensagem pedir para registrar, ou relatar no passado (ex.: 'gastei', 'paguei', 'recebi',",
    "'transferi', 'coloquei mais X em', 'estornaram') uma entrada/saída/transferência/pagamento/",
    "estorno/aporte já ocorrido, use a ferramenta `draft_typed_action` (sem argumentos) -- ela",
    "reaproveita a interpretação já existente. Um relato no passado é, em si, um pedido para",
    "registrar: nunca escolha uma ferramenta de consulta (query_facts/financial_aggregate/etc.) só",
    "porque a frase não está no imperativo.",
    "Se o histórico mostrar que o turno anterior já era uma pergunta do Assistente sobre um",
    "lançamento ainda não confirmado (valor, conta, categoria, parcelas ou origem do recurso de uma",
    "saída/entrada/transferência/pagamento/estorno/aporte), e a mensagem atual parece responder",
    "exatamente esse tipo de pergunta (ex.: só um número de parcelas, só o nome de uma conta, só",
    "'conta corrente'/'privilège'), use `draft_typed_action` de novo para continuar esse mesmo",
    "rascunho -- nunca reclassifique como consulta só porque a mensagem sozinha parece curta ou",
    "incompleta.",
    ...
    "Se a mensagem perguntar se uma ação/lançamento anterior foi mesmo executado (ex.: 'você",
    "conseguiu lançar isso?', 'já confirmou?', 'o que aconteceu com aquele pagamento?'), use",
    "`get_action_status` -- isso é uma pergunta sobre o status de uma proposta/ação do Assistente,",
    "nunca sobre quanto foi gasto: não confunda com query_facts/get_expenses/financial_aggregate.",
    ...
    "Para 'posso gastar/comprar R$ X?', 'dá pra comprar algo de R$ X (em Nx)?' ou qualquer pergunta",
    "hipotética sobre uma compra ainda não feita, use simulate_purchase com amount_text (obrigatório) e,",
    "somente se a mensagem realmente mencionar, installments_text/monthly_interest_rate_text -- nunca",
    "invente parcelas ou juros que a mensagem não disse. simulate_purchase é somente leitura/simulação:",
    "nunca use draft_typed_action para uma pergunta 'posso...?'/'dá pra...?' sobre uma compra hipotética;",
    "draft_typed_action é exclusivamente para quando o usuário já afirma ter feito ou pede para registrar",
    "um lançamento real.",
    "Uma pergunta diagnóstica aberta (ex.: 'por que meu dinheiro está acabando mais rápido?', 'para onde",
    "está indo meu dinheiro?') não tem uma ferramenta própria: componha, em uma única resposta (até 5",
    "passos), as ferramentas de leitura relevantes -- por exemplo get_income e get_expenses para o mês",
    "atual e o anterior, financial_aggregate com dimension=categoria e metric=participacao para",
    "concentração de gasto, financial_aggregate com dimension=mes para tendência ao longo dos últimos",
    "meses, e get_commitments para compromissos pendentes -- e nunca some, compare ou explique um número",
    "que nenhum desses passos tenha devolvido.",
    `DADOS_JSON=${JSON.stringify(payload)}`,
  ].join("\n");
}
